...
class Vmess(User):
    ...
    def __str__(self):
        ...
        result = '''{email}
                     ...
                     Network: {network}'''.format(self=self, uuid=self.password, email=email, network=self.stream()).strip() + "\n"
        return result
...
{node}{tfo}
{dyp}
{link}'''.format(self=self, color_ip=ColorStr.fuchsia(self.ip), port_way=port_way, node=node,tfo=tfo, dyp=dyp,tls=tls, link=node.link(self.ip, int(self.port), self.tls))
        return result
