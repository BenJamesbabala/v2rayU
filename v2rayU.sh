#!/bin/bash
# Author: DockerCS
# github: https://github.com/DockerCS/v2rayU
...
#安装方式0: 全新安装, 1:保留配置更新 , 2:仅更新v2rayU源码
INSTARLL_WAY=0
...
while [[ $# > 0 ]];do
    ...
    case $key in
        ...
        -v|--version)
        UPDATE_VERSION="$2"
        echo -e "更新v2rayU到 $(colorEcho ${BLUE} $UPDATE_VERSION) 版本\n"
        shift
        ;;
        -k|--keep)
        INSTARLL_WAY=1
...
checkUpdate(){
    LASTEST_VERSION=$(curl -H 'Cache-Control: no-cache' -s "https://api.github.com/repos/DockerCS/v2rayU/releases/latest" | grep 'tag_name' | cut -d\" -f4)
    ...
    if [[ -e /usr/local/bin/v2ray ]];then
        ...
        if [[ ! -z $VERSION_TEMP_VALUE ]]; then
            ...
            if [[ ! -z $UPDATE_VERSION && $UPDATE_VERSION == $CURRENT_VERSION ]];then
                echo -e "v2rayU当前版本: $(colorEcho $GREEN $CURRENT_VERSION), 已是指定版本!!!"
                IS_LATEST=1
                return
            fi
            if [[ -z $UPDATE_VERSION && $FORCE == 0 && $INSTARLL_WAY != 0 && $LASTEST_VERSION == $CURRENT_VERSION ]]; then
                echo -e "v2rayU当前版本: $(colorEcho $GREEN $CURRENT_VERSION), 已是最新!!!"
                IS_LATEST=1
                return
            fi
        fi
...
help(){
    echo "source v2rayU.sh [-h|--help] [-k|--keep] [-c|--code] [-f|--force] [--remove]"
    echo "  -h, --help           Show help"
    echo "  -k, --keep           keep the v2ray config.json to update"
    echo "  -c, --code           only update v2rayU code"
    echo "  -f, --force          force to update v2rayU lastest code"
    echo "  -v, --version        update v2rayU to special version"
    echo "      --remove         remove v2ray && v2rayU"
...
updateProject() {
    ...
    elif [[ -e $APP_PATH/v2rayU.conf ]];then
        TEMP_VALUE=$(cat $APP_PATH/v2rayU.conf|grep domain|awk 'NR==1')
        DOMAIN=${TEMP_VALUE/*=}
    fi
    ...
    [[ ! -z $DOMAIN ]] && sed -i "s/^domain.*/domain=${DOMAIN}/g" $APP_PATH/v2rayU.conf
...
profileInit() {
    ...
    #加入v2rayU模块搜索路径
    [[ -z $(grep v2rayU ~/$ENV_FILE) ]] && echo "export PYTHONPATH=$PYTHONPATH:$APP_PATH" >> ~/$ENV_FILE && source ~/$ENV_FILE
